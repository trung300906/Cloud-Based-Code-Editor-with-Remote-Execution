...
const MINIO_ENDPOINT =
  process.env.MINIO_ENDPOINT || "http://100.84.67.110:9000";
...
async function main() {
  ...
  console.log(
    `[WorkerNode] MinIO endpoint=${MINIO_ENDPOINT} bucket=${MINIO_BUCKET}`,
  );
  ...
  const poolManager = new MasterPoolManager({
    ...
    onQueuedJobError(err, queued) {
      ...
      if (sock?.writable) {
        sock.write(
          buildFrame(TYPE.ERROR, rid, `Job failed in queue: ${err.message}`),
        );
      }
  ...
  function handleExec(socket, requestId, bodyBuf) {
    ...
    try {
      ...
      if (socket.writable)
        socket.write(buildFrame(TYPE.ERROR, rid, "Bad RUN payload JSON"));
      pendingRequests.delete(rid);
      return;
    }
      ...
      if (socket.writable) {
        socket.write(
          buildFrame(
            TYPE.ERROR,
            rid,
            "Missing fields. Received: " + JSON.stringify(spec),
          ),
        );
      }
      pendingRequests.delete(rid);
    ...
    poolManager
      .dispatchJob(jobPayload, async (container, job) => {
        ...
        try {
          ...
          if (job.spec.projectId && job.spec.projectId !== "null") {
            await pullWorkspaceFromMinio(
              job.jobDir,
              job.spec.ownerId,
              job.spec.projectId,
            ).catch((err) => {
              console.warn(
                `[WorkerNode] Failed to pull workspace: ${err.message}`,
              );
            });
          }
          ...
          console.log(
            `[WorkerNode] Dispatched job ${job.rid} to container ${container.id}`,
          );
          ...
          if (job.spec.language === "cpp" || job.spec.language === "c++") {
            ...
            job.spec.language === "python" ||
            job.spec.language === "py"
          ) {
          ...
          console.log(
            `[WorkerNode] Running command: ${cmd} in /workspace/${safeRid}`,
          );
          ...
          dockerContainer.modem.demuxStream(
            stream,
            {
              write: (chunk) => {
                if (sock.writable)
                  sock.write(buildFrame(TYPE.RESULT, job.rid, chunk));
              },
            }, // stdout
            {
              write: (chunk) => {
            ...
            if (sock.writable)
              sock.write(
                buildFrame(
                  TYPE.ERROR,
                  job.rid,
                  `Timeout after ${EXEC_TIMEOUT_MS}ms`,
                ),
              );
          } else {
            if (sock.writable)
      ...
      .catch((err) => {
        ...
        if (sock?.writable)
          sock.write(
            buildFrame(TYPE.ERROR, rid, `Dispatch failed: ${err.message}`),
          );
        pendingRequests.delete(rid);
      });
  }
  ...
  const server = net.createServer((socket) => {
    ...
    const parser = new FrameParser(
      (type, payload) => {
        if (type !== TYPE.RUN && type !== TYPE.INPUT && type !== TYPE.LINT)
          return;
    ...
    socket.on("error", (err) =>
      console.error(`[WorkerNode] socket error (${remote}):`, err.message),
    );
    socket.on("close", () => {
      let evicted = 0;
      ...
      if (evicted)
        console.log(
          `[WorkerNode] disconnected: ${remote}, evicted ${evicted} pending request(s)`,
        );
