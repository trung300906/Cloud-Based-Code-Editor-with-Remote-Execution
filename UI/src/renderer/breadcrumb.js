...
export function updateBreadcrumb(filePath, pane) {
  ...
  if (!filePath) {
    bar.innerHTML = `${BC_ICON.__default__}<span class="bc-segment bc-file" style="color:#858585;font-style:italic">untitled</span>`;
    return;
  }
  ...
  if (allParts.length <= 4) {
    segments = allParts.map((name, i) => ({
      name,
      isFile: i === allParts.length - 1,
    }));
  } else {
    segments = [
